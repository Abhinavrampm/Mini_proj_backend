...
router.post('/getcalorieintakebydate', authTokenHandler, async (req, res) => {
    ...
    if (!date) {
        let date = new Date();   //apr 1 2024 12:00:00
...
router.delete('/deletecalorieintake', authTokenHandler, async (req, res) => {
    ...
    user.calorieIntake = user.calorieIntake.filter((item) => {
        return item.date.toString()!== new Date(date).toString(); //convert the date from front end to compare with the backend value(same in every api to delete)
    })
    await user.save();
...
router.get('/getgoalcalorieintake', authTokenHandler, async (req, res) => {
    ...
    if (gender == 'male') {
        BMR = 88.362 + (13.397 * weightInKg) + (4.799 * heightInCm) - (5.677 * age) //calculation of bmr used for tracking calorie to maintain weight
