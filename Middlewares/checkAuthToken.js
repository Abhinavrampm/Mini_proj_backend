...
function checkAuth(req, res, next) {
    ...
    jwt.verify(authToken, process.env.JWT_REFRESH_SECRET_KEY, (err, decoded) => {
        if (err) {
            ...
            jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET, (refreshErr, refreshDecoded) => {
                if (refreshErr) {
                    ...
                    const newAuthToken = jwt.sign({ userId: refreshDecoded.userId }, process.env.JWT_REFRESH_SECRET_KEY, { expiresIn: '10m' });
